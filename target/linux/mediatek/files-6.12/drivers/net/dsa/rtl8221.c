...
static void Rtl8226b_PHYmodeEEE_set(struct mii_bus *bus, int hDevice,int on_off)
{
    u16 phydata = 0;
    if(on_off){
...
static int gsw_link_thread(void *resv)
{
    ...
    msleep(20000); // wait for system start
    for(;;) {      
         uint16_t phydata;
         bool blinkOk = false;
         ...
         if (gsw_phy_link != blinkOk) {
              if (blinkOk) {
		    printk("rtl8221 link up\n");
              } else {
		    printk("rtl8221 link down\n");
              }
         }
